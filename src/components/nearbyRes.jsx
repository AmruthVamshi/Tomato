...
class NearbyRes extends React.Component {
    state = {}
     componentDidMount(){
         this.props.location()
        setTimeout(()=>{
            let currentLocation=this.props.LocationState.getLocation.location
            let url=`https://developers.zomato.com/api/v2.1/geocode?lat=${currentLocation[0]}&lon=${currentLocation[1]}`
        let options = {
            headers:{
        ...
        .then(restaurants_response=>{
            let restaurants = restaurants_response.nearby_restaurants.map( restaurant => {
                return(
...
export default connect(mapStateToProps,mapDispatchToProps)(NearbyRes);
